...
(function($){
	...
	var VideoCover = function(target, options ){
		var componentObj = {
			...
			methods:{
				init:function(){
					if(!componentObj.methods.isMobile()){
						...
						if(componentObj.image != null){
							componentObj.methods.setImage();
						}
					}
				},
				...
				setVideo: function(){
					...
					if(componentObj.videoType === "vimeo"){
						...
						componentObj.video_id+'?'+autoplay+'&loop=1'+loop
					...
					$(componentObj.frameTag).css({
						"width":(componentObj.control)?"100%":"120%",
						"height": (componentObj.control)?"100%":"120%",
						"position": "absolute",
						"top": (componentObj.control)?"0":"-10%",
						"left": (componentObj.control)?"0":"-10%",
						"bottom": "0px",
						"right": "0px",
						"margin": "auto",
						"z-index": "1"
				...
				setImage: function(){
					var image = '<img src="'+componentObj.image+'">';
					var imageElement = $(image).appendTo(target);
					$(imageElement).css({
						"display":"block",
						"max-width": "100%",
						"height": "auto",
